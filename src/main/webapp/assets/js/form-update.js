...
student_form.addEventListener('submit', async (e) => {
    ...
    if (student_form.checkValidity() === true) {
        ...
        let response = await fetch('api/company/update', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                company_id: document.getElementById('id').value,
                hr_name: document.getElementById('last_name').value,
                company_name : document.getElementById('first_name').value,
